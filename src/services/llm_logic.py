...
class LLMService:
    ...
    def analyze(self, text: str) -> TTNDocument:
        prompt = f"""
         ТЫ — РОБОТ-ОЦИФРОВЩИК. Твоя задача: оцифровать ТТН Беларуси в JSON БЕЗ СОКРАЩЕНИЙ.
        ...
        5. SHIPPER/CONSIGNEE: Не путай местами. Отправитель (Shipper) обычно Диасенс, Получатель (Consignee) обычно Лодэ. Если адрес занимает 5 строк - перенеси ВСЕ 5 строк. Не выкидывай индексы и города!
        6. shipper_unp и consignee_unp: Найди 9-значные коды для обеих сторон.
        7. unit: строго [шт, уп, набор, флак, упак] или пусто. "yn" считай "уп", "WT" — "шт". Для реагентов ищи "набор" или "флак".
